...
#     (r'^account/passwordreset/confirm/(?P<uidb64>[0-9A-Za-z]+)-(?P<token>.+)/$','password_reset_confirm', name='password_reset_confirm',
...
from ecm.views.account.forms import PasswordChangeForm, PasswordResetForm, PasswordSetForm
from ecm.views.ajax import celestial, moons, solarsystem
from ecm.views.account import home, signup
from ecm.views import common, api
...
urlpatterns += [
    ...
    url(r'^account/passwordreset/confirm/(?P<uidb64>[0-9A-Za-z]+)-(?P<token>.+)/$', auth_views.password_reset_confirm,
            {
                'template_name': 'ecm/auth/password_reset_confirm.html',
                'set_password_form': PasswordSetForm
            }
...
urlpatterns += [
    ###########################################################################
    # ECM AUTH + USER PROFILE VIEWS
    url(r'^account/$', home.account),
    url(r'^account/addapi/$', home.add_api),
    url(r'^account/deleteapi/(\d+)/$', home.delete_api),
    url(r'^account/deletecharacter/(\d+)/$', home.delete_character),
    url(r'^account/editapi/(\d+)/$', home.edit_api),
    url(r'^account/binding/add/(\d+)/$', home.add_binding),
    url(r'^account/binding/delete/(\d+)/$', home.delete_binding),

    url(r'^account/create/$', signup.create_account),
    url(r'^account/activate/(\w+)/$', signup.activate_account),
]
...
urlpatterns += [
    ...
    # COMMON VIEWS
    url(r'^$', common.home),
    url(r'^editmotd/$', common.edit_motd),
    url(r'^editapi/$', common.edit_apikey),
]
...
urlpatterns += [
    ...
    url(r'^api/bindings/(\w+)/users/$', api.user_bindings),
    url(r'^api/bindings/(\w+)/groups/$', api.group_bindings),
]

urlpatterns += [
    ###########################################################################
    # AJAX QUERY VIEWS
    url(r'^ajax/celestials/$', celestial.list),
    url(r'^ajax/solarsystems/$', solarsystem.list),
    url(r'^ajax/moons/$', moons.list),
]
